...
class ProductIntersection:
    def __init__(
        ...
        resampling_mapping = {
            method.name: method for method in rasterio.enums.Resampling
        }
    ...
    def has_intersection(self):
        ...
        """
        times1 = (
            self.meta1.start_date - self.delta_time_np,
            self.meta1.stop_date + self.delta_time_np,
        )
        times2 = (
            self.meta2.start_date - self.delta_time_np,
            self.meta2.stop_date + self.delta_time_np,
        )
        ...
        if (self.meta1.acquisition_type == "truncated_grid") and (
            ...
            (self.meta1.acquisition_type == "truncated_grid")
            and (self.meta2.acquisition_type == "daily_regular_grid")
        ) or (
            (self.meta2.acquisition_type == "truncated_grid")
            and (self.meta1.acquisition_type == "daily_regular_grid")
        ):
            return self.intersection_drg_truncated_grid()
        elif (
            (self.meta1.acquisition_type == "truncated_grid")
            and (self.meta2.acquisition_type == "swath")
        ) or (
            (self.meta2.acquisition_type == "truncated_grid")
            and (self.meta1.acquisition_type == "swath")
        ):
            return self.intersection_swath_truncated_grid()
        ...
        elif (self.meta1.acquisition_type == "swath") and (
            ...
            (self.meta1.acquisition_type == "truncated_swath")
            and (self.meta2.acquisition_type == "swath")
        ) or (
            (self.meta2.acquisition_type == "truncated_swath")
            and (self.meta1.acquisition_type == "swath")
        ):
            return self.intersection_truncated_swath_swath()
    ...
    def intersection_with_model(self):
        ...
        def rasterize_polygon(open_acquisition, polygon):
            if open_acquisition.acquisition_type == "model_regular_grid":
                ...
                lon_res = abs(
                    ...
                    - open_acquisition.dataset[lon_name][0]
                )
                lat_res = abs(
                    open_acquisition.dataset[lat_name][1]
                    - open_acquisition.dataset[lat_name][0]
                )
                out_shape = [
                    len(open_acquisition.dataset[lat_name]),
                ...
                raise ValueError(
                    "`rasterize_polygon` only can be applied on daily regular grid acquisition"
                )
        ...
        def geographic_intersection(open_acquisition, polygon=None):
            if open_acquisition.acquisition_type == "model_regular_grid":
                ...
                raise ValueError(
                    "`geographic_intersection` only can be applied on model regular grid acquisition"
                )
        ...
        if self.product_generation:
            ...
            if other_meta.acquisition_type == "model_regular_grid":
                ...
                corners = [
                    (min(unique_lon), min(unique_lat)),
                    (min(unique_lon), max(unique_lat)),
                    (max(unique_lon), max(unique_lat)),
                    (max(unique_lon), min(unique_lat)),
                ]
            ...
            #  the co-location is with a truncated grid
            if other_meta.acquisition_type == "truncated_grid":
                self._datasets[model.product_name] = geographic_intersection(
                    model, polygon=other_meta.footprint
                )
    ...
    def intersection_drg_truncated_grid(self):
        """
        Method that verifies if there is an intersection between a daily regular grid product and a truncated grid.
        This method can fill attributes like `self._datasets` (with the datasets after temporal and
        spatial intersection) and `self._common_footprint`.
        ...
        if (self.meta1.acquisition_type == "truncated_grid") and (
            ...
            daily = self.meta2
        elif (self.meta2.acquisition_type == "truncated_grid") and (
            self.meta1.acquisition_type == "daily_regular_grid"
        ):
            ...
            raise ValueError(
                "intersection_drg_truncated_grid only can be used with a daily regular grid \
        ...
        if daily.has_orbited_segmentation:
            ...
            for orbit in daily.dataset[daily.orbit_segment_name].data:
                ...
                sub_daily.dataset = sub_daily.dataset.sel(
                    **{sub_daily.orbit_segment_name: orbit}
                )
            ...
            self._datasets[daily.product_name] = xr.concat(
                li, dim=daily.orbit_segment_name
            )
    ...
    def intersection_swath_truncated_grid(self):
        """
        ...
        if (self.meta1.acquisition_type == "truncated_grid") and (
            ...
            raise ValueError(
                "intersection_swath_truncated_grid only can be used with a swath \
                                acquisition and a truncated one"
            )
        ...
        if swath.has_orbited_segmentation:
            ...
            for orbit in swath.dataset[swath.orbit_segment_name]:
                ...
                # Select orbit in the dataset of sub_daily
                sub_swath.dataset = sub_swath.dataset.sel(
                    **{sub_swath.orbit_segment_name: orbit}
                )
    ...
    def coloc_resample_gridded(self):
        """
        ...
        dataset1 = dataset1.rename(
            {meta1.longitude_name: "x", meta1.latitude_name: "y"}
        )
        dataset2 = dataset2.rename(
            {meta2.longitude_name: "x", meta2.latitude_name: "y"}
        )
        ...
        if (
            dataset1.x[0].values < 0 and dataset1.x[len(dataset1.x) - 1].values > 180
        ) or (
            dataset2.x[0].values < 0 and dataset2.x[len(dataset2.x) - 1].values > 180
        ):
            meridian_datasets = True
            logger.info(
                "datasets cross Greenwich Meridian, dataset coordinated will be modified after reprojection."
            )
        else:
        ...
        logger.info(
            "Reprojecting dataset with higher resolution to make it match with the lower resolution one."
        )
        if (
            pixel_spacing_lon1 * pixel_spacing_lat1
            <= pixel_spacing_lon2 * pixel_spacing_lat2
        ):
            # some product don't have the same resolution, so we apply the same resolution by specifying a resampling
            dataset1 = dataset1.rio.reproject_match(
                dataset2, resampling=self.resampling_method
            )
            reprojected_dataset = "dataset1"
            logger.info("dataset1 reprojected")
        else:
            # some product don't have the same resolution, so we apply the same resolution by specifying a resampling
            dataset2 = dataset2.rio.reproject_match(
                dataset1, resampling=self.resampling_method
            )
        ...
        dataset1 = dataset1.rename(
            {"x": meta1.longitude_name, "y": meta1.latitude_name}
        )
        dataset2 = dataset2.rename(
            {"x": meta2.longitude_name, "y": meta2.latitude_name}
        )
        ...
        return {
            "meta1": dataset1,
            "meta2": dataset2,
            "reprojected_dataset": reprojected_dataset,
        }

    def coloc_resample_swath(self):
        """
        Resample 2 satellite swath (2D lon and lat) datasets from `self.meta1`and `self.meta2`. If a dataset exists in `self._datasets`
        (it means that a meta dataset has been intersected temporally and spatially), so this one is chosen.
        ...
        ds2 = remove_nat(meta2, dataset2)

        def only_keep_required_vars(meta, ds):
            unecessary_vars = meta.unecessary_vars_in_coloc_product
            ds = ds.drop_vars(
                [var for var in ds.variables if var in unecessary_vars], errors="ignore"
            )
            return ds

        # Remove variables because they may be unadapted to colocation
        ds1 = only_keep_required_vars(meta1, ds1)
        ds2 = only_keep_required_vars(meta2, ds2)

        lon_name_1 = meta1.longitude_name
        lat_name_1 = meta1.latitude_name
        lon_name_2 = meta2.longitude_name
        lat_name_2 = meta2.latitude_name

        lon_1 = ds1[lon_name_1].values
        lat_1 = ds1[lat_name_1].values
        ...
        for n in ds1.data_vars:
            data_vars_1[n] = ds1[n].astype("float64").values

        data_vars_2 = Dict.empty(
            key_type=types.unicode_type, value_type=types.float64[:, :]
        )
        for n in ds2.data_vars:
        ...
        if lon_1_delta > lon_2_delta:
            ...
            lat_reduced = lat_1_reduced
        else:
            lon_reduced = lon_2_reduced
            lat_reduced = lat_2_reduced

        colocated_data_2 = Dict.empty(
            key_type=types.unicode_type, value_type=types.float64[:, :]
        )
        for n in data_vars_2:
            colocated_data_2[n] = np.full(lon_reduced.shape, np.nan)

        colocated_data_1 = Dict.empty(
            key_type=types.unicode_type, value_type=types.float64[:, :]
        )
    ...
    def coloc_resample(self):
        ...
        gridded_types = ["truncated_grid", "daily_regular_grid", "model"]
        swath_types = ["truncated_swath", "swath"]
        if (
            meta1.acquisition_type in gridded_types
            and meta2.acquisition_type in gridded_types
        ):
            return self.coloc_resample_gridded()
        elif (
            meta1.acquisition_type in swath_types
            and meta2.acquisition_type in swath_types
        ):
            return self.coloc_resample_swath()
        else:
            raise ValueError(
                f"Unknown acquisition_type: {meta1.acquisition_type} or {meta2.acquisition_type}"
            )

    def get_common_zone_gridded(self):
        """
        ...
        poly_intersection = shapely.ops.transform(
        ...
        if reprojected_dataset == "dataset1":
            ...
            geometry_mask = rasterio.features.geometry_mask(
                [poly_intersection],
                out_shape=(dataset2[lat_name].shape[0], dataset2[lon_name].shape[0]),
        ...
        dataset1_common_zone = dataset1_common_zone.assign_attrs(
            {"polygon_common_zone": str(poly_intersection)}
        )
        ...
        dataset2_common_zone = dataset2_common_zone.where(
            np.isfinite(dataset2_common_zone[meta2.time_name]), drop=True
        )

        logger.info("Done getting common zone.")
        return dataset1_common_zone, dataset2_common_zone

    def get_common_zone_swath(self):
        """
        For swath types, datasets already are on a common reduced zone.
        """
        dataset1 = self.resampled_datasets["meta1"]
        dataset2 = self.resampled_datasets["meta2"]
    ...
    def get_common_zone(self):
        ...
        if (
            ...
            return self.get_common_zone_gridded()
        elif (
            meta1.acquisition_type in swath_types
            and meta2.acquisition_type in swath_types
        ):
            return self.get_common_zone_swath()
        else:
            raise ValueError(
                f"Unknown acquisition_type: {meta1.acquisition_type} or {meta2.acquisition_type}"
            )
    ...
    def format_datasets(self):
        ...
        def apply_attributes_changes(meta, ds):
            ...
            ds.attrs = {
                attrs_rename_func(attr): ds.attrs[attr]
            ...
            if (
                meta.acquisition_type == "truncated_grid"
                or meta.acquisition_type == "truncated_swath"
            ):
                ...
                if hasattr(meta, "footprint"):
                    ...
                    footprint = get_footprint_from_ll_ds(
                        meta, ds, self.start_date, self.stop_date
                    )
        ...
        if are_dimensions_empty(dataset1) or are_dimensions_empty(dataset2):
            raise ValueError(
                "There are not enough common points to create the co-location product or format datasets"
            )
        # Attributes changes
    ...
    def merge_datasets(self):
        ...
        def ws_analysis_attributes():
            ...
            if counted_points > 5:
                ...
                correlation_coefficient = xr.corr(
                    dataset1["wind_speed_1"], dataset2["wind_speed_2"]
                ).values
            else:
                dict_ws_analysis["Bias"] = np.nan
                dict_ws_analysis["Standard deviation"] = np.nan
                correlation_coefficient = 0

            dict_ws_analysis["scatter_index"] = calculate_scatter_index(
                dataset1["wind_speed_1"], dataset2["wind_speed_2"]
            )
        ...
        def get_common_attrs():
            attrs = {}
            start1, stop1 = (
                dataset1.attrs["measurementStartDate_1"],
                dataset1.attrs["measurementStopDate_1"],
            )
            start2, stop2 = (
                dataset2.attrs["measurementStartDate_2"],
                dataset2.attrs["measurementStopDate_2"],
            )
            attrs["time_difference"] = str(mean_time_diff(start1, stop1, start2, stop2))
            attrs["polygon_common_zone"] = poly_common_zone()
            attrs["area_intersection"] = str(
                get_polygon_area_in_km_squared(attrs["polygon_common_zone"])
            )
